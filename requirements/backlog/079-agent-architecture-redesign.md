...
### Stage 4: 后置验证 + 修正循环（Reflect-Refine Loop）

**参考：**
- Reflexion（arXiv 2303.11366）— 失败后生成自然语言反思指导重试
- Run-Verify-Reflect（RVR）— 闭环：运行→验证→反思→重试
- AWS Evaluator Reflect-Refine — 评估器检查→反馈→精炼输出
- agentic-patterns.com — "重复直到通过阈值或重试预算耗尽"
- tianpan.co — "LLM 验证 LLM = 共享盲点"，必须用代码验证
...
Stage 3 输出
    ↓
[验证器] — 代码比对 output vs ExecutionRecord
    ├── 通过 → 输出给用户 ✅
    └── 发现问题 → 进入修正循环
                    ↓
              [反馈修正] — 告诉 Stage 3 的 LLM：
                "你的回复说了 X，但实际执行结果是 Y。请修正。"
                    ↓
              Stage 3 重新生成
                    ↓
              [再次验证]
...
async def stage_4_verify_and_refine(output: str, record: ExecutionRecord, user_question: str) -> str:
    """验证 + 修正循环"""
    for attempt in range(MAX_REFLECT_RETRIES):
        issues = verify_output(output, record)  # 代码级验证
        if not issues:
            return output  # 通过，直接输出

        if attempt < MAX_REFLECT_RETRIES - 1:
            # 反馈修正：把问题告诉 LLM 让它重来
            correction_prompt = f"""你的上一次回复有以下问题：
{issues}

实际执行结果是：
{format_execution_record(record)}

请重新生成回复，确保只引用实际执行结果中的数据。"""
            output = await call_llm_simple(correction_prompt, max_tokens=300)
        else:
            # 重试耗尽 → 代码兜底
            output = generate_safe_fallback(record, user_question)
            output += "\n\n⚠️ 系统验证：此回复基于实际工具执行结果生成。"

    return output


def generate_safe_fallback(record: ExecutionRecord, question: str) -> str:
    """代码级安全回复 — 100% 基于执行记录，零幻觉"""
    if not record.results:
        return "我尝试处理你的请求，但没有获取到有效数据。需要我换个方式试试吗？"
...
| 决策 | 选择 | 理由 |
|------|------|------|
| 最大重试次数 | 2 次 | 超过 2 次说明 LLM 根本不看验证反馈，浪费 token |
| 兜底方式 | 代码模板化 | 不信任 LLM，从数据生成，100% 准确 |
| 验证器类型 | 纯代码（不用 LLM） | LLM 验证 LLM = 共享盲点 |
| 反馈格式 | 精确指出哪个数字/声明错了 + 给出正确数据 | 不笼统说"你错了" |

**行业警告：**

> "用 LLM 验证 LLM 的输出，验证器和生成器共享相同的盲点。如果模型倾向于生成某个错误，它也倾向于验证通过那个错误。" — tianpan.co

**所以验证器必须是代码级的，不能用 LLM。**
