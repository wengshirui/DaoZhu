...
| `exit` | id | 角色退场（滑出屏幕） |
| `move` | id, x, y? | **角色移动**（重要！不要让角色一直站着不动） |
| `arm` | id, arm | 改变手臂姿态 |
| `emoji` | id, e | 角色头顶弹出 emoji |
| `dialogue` | who, text | 对话（**who 必须用中文角色名**，如"父亲"，不要用 id） |
| `narr` | text | 底部深色旁白条 |
...
1. **dialogue 的 who 必须用中文 label**，不要用英文 id！
   - ✅ `{"who": "父亲", "text": "..."}`
   - ❌ `{"who": "fuqin", "text": "..."}`

2. **角色必须有动作（move）**，不要让角色一直站在原地！
   - 每 3-4 个事件至少有一次 move
   - 对话前后角色要走近/走远
   - 例：父亲去买橘子 → move 到远处 → 回来 → move 回近处
