...
const App = {
  ...
  _initLifecycle() {
    ...
    setInterval(() => this._updateLifecycle(), 10000);

    document.getElementById('btn-kill-agent').addEventListener('click', async () => {
      const reason = prompt('为什么要结束当前 Agent？\n（理由会帮助下一代做得更好）');
      if (!reason) return;
      if (!confirm(`确定终结第 ${this._currentGen} 代？`)) return;
      ...
      try {
        ...
        if (data.success) {
          alert(`第 ${data.dead_generation} 代已终结\n\n遗言：${data.farewell}\n\n第 ${data.new_generation} 代已诞生`);
          this._updateLifecycle();
          Panel.addLog('info', `💀 第 ${data.dead_generation} 代 → 第 ${data.new_generation} 代`);
        }
      } catch (e) {
