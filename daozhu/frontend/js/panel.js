          ...
          if (msg.role === 'tool_call') {
            ...
            try {
              ...
              const steps = tools.map(t => {
                ...
                const statusText = t.status === 'ok' ? '完成' : (t.error || '失败');
                return `<div class="tool-panel__step ${cls}"><span class="tool-panel__step-icon">${icon}</span> <span class="tool-panel__step-name">${t.tool}</span> <span class="tool-panel__step-status">${statusText}</span></div>`;
              }).join('');
